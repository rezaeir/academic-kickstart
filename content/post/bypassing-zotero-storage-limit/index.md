...
## A better way to sync Zotero files between computers in your local network

Actually I was using this Zotero-Google Drive method for a while, but because I use VPNs, the syncing process wasn't working as automatically as it should. So, I searched for another method which I mention briefly. [SyncToy](https://www.microsoft.com/en-us/download/details.aspx?id=15155) is a tool from Microsoft which is designed for syncing folder contents. The configuration is pretty easy and self-explanatory. I explain it briefly here!

1. revert back both of the Zotero storage directories in your devices to their default in Zotero preferences (remember to copy the files from your google drive to the default directory, first).
2. Share the Zotero folder or specifically the storage folder in this directory in one of your devices with the other by right-clicking on the folder and using `Give access to` option.
3. Install the SyncToy on the other device and set the `zotero/storage` folder from the local drive and the same folder from the local network in the second device.
4. Run the software each time that you need to files to be synced and it will sync them.
